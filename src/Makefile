...
LDFLAGS = $(CURL_LIBS) $(LIBXML2_LIBS) $(FUSE_LIBS) -lpthread -lcrypto -lssl -lcurl
...
.PHONY: compress-example 
compress-example: $(BUILD)/bin/compress-example

CLOUDFS_OBJS = $(BUILD)/obj/compressapi.o \
							 $(BUILD)/obj/compress-example.o

$(BUILD)/bin/compress-example: $(CLOUDFS_OBJS)
	$(QUIET_ECHO) $@: Building executable
	@ mkdir -p $(dir $@)
	$(VERBOSE_SHOW) gcc -o $@ $^ $(LDFLAGS) $(LIBRARY)
